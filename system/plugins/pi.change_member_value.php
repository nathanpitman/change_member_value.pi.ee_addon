...
$plugin_info = array(
  'pi_name' => 'Change Member Value',
  'pi_version' => '1.0',
  'pi_author' => 'Nine Four',
  'pi_author_url' => 'http://ninefour.co.uk/labs',
  'pi_description' => 'Allows you to change a custom member profile field value from within a front end page template.',
  'pi_usage' => Change_member_value::usage()
  );

class Change_member_value
{

	// Config
	var $invalid_member = "<strong>Error:</strong> The member id provided is invalid.";
	var $invalid_value = "<strong>Error:</strong> The value provided is invalid.";
	...
	function link()
	{
		...
		if (!$TMPL->fetch_param('member_id') OR !$TMPL->fetch_param('field') OR !$TMPL->fetch_param('value') OR !$TMPL->fetch_param('template') OR !$TMPL->fetch_param('return_url')) {
			return "Invalid change_member_value plugin usage.";
		} else {
		
			// Collect params
			$member_id = $TMPL->fetch_param('member_id');
			$field = $TMPL->fetch_param('field');
			...
			$change_string = base64_encode($member_id."~".$field."~".$value."~".$return_url);
	...
	function change()
	{
		...
		// Do the member field update!
		$query = $DB->query("UPDATE exp_member_data set '".$field."' = '".$value."' WHERE member_id = '".$member_id."'");
		...
		// If no error redirect or do nothing, depending on usage type
			...
			$LOG->log_action(array("Member Data Changed"));
	...
	ob_start(); 
	?>
	The change member value plugin allows you to change the value of a custom member profile field through the front end page templates. You can use the plug-in in two ways, to generate a 'link' which a users clicks on to effect the change, or have the change implemented immediately upon visiting a template with the relevant 'change' tag in place:

GENERATE CHANGE LINK - {exp:change_member_value:link}
...
This tag is used in pairs and generates the URL to the member group change page. There are four available parameters with the link function:
 ...
 - field (Required) - The name of the member field that you wish to assign members to. E.g. 'm_field_id_1'
 - value (Required) - The value which you wish to insert into the specified field. E.g. 'Badgers'
 - template (Required) - The name of the template containing the {exp:change_member_value:change} plugin code. E.g. site/change_member_value
...
Example usage: 
{exp:change_member_value:link member_id="{member_id}" field="m_field_id_1" value="Badgers" template="site/change_member_value" confirm_message="Are you sure you want to change this field value?" return_url="/site/index"}Change my field value...{/exp:change_member_value:link}


CHANGE THE MEMBER FIELD VALUE - {exp:change_member_value:change}

This tag should be embedded in a special 'change member value' template. You can wrap as little or as much HTML around the tag as you like to ensure that the page is styled to suit your site. If you are using this tag with no parameters you must have linked to this template using the 'link' tag and have provided a return_url value in the link tag. To use this tag on it's own you must provide a field and value and can optionally provide a member_id. If no member_id is provided then the plug-in will by default attempt to modify the currently logged in members group.

This function only has three parameters:
 ...
 - field (Optional) - Optional but must be specified if you are using the change tag in a standalone form to effect an instant member field value change on template load.
 - value (Optional) - Optional but must be specified if you are using the change tag in a standalone form to effect an instant member field value change on template load.
 
Example usage:
 
{exp:change_member_value:change field="m_field_id_1" value="Badgers"}

IMPORTANT NOTE

Given that this plug-in allows the modification of member data you should take great care to ensure that everything works as intended. Care should also be exercised with the link tag as users will receive no prompt if javascript is not enabled in their browser (or if confirm_message is not specified as a parameter). 
